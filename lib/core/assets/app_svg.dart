class AppVector {
  static const String basePath = "assets/vector/";
  static const String spotifyLogo = "${basePath}spotify_logo$format";
  static const String darkThemeLogo = "${basePath}dark_theme$format";
  static const String lightThemeLogo = "${basePath}light_theme$format";
  static const String bottomPattern = "${basePath}bottom_pattern$format";
  static const String topPattern = "${basePath}top_pattern$format";
  static const String searchIcon = "${basePath}search$format";
  static const String spotifyLogoHome = "${basePath}spotify_logo_home$format";

  static const String format = ".svg";
}
